...
library(data.table)
library(Matrix)
mat <- readMM("gene_sorted-matrix.mtx") 

mat <- as(mat,"dgCMatrix")
feature.names <- read.delim("genes.tsv",header = FALSE,stringsAsFactors = FALSE)
barcode.names <- read.delim("barcodes.tsv", header = FALSE,stringsAsFactors = FALSE)
colnames(mat) = barcode.names$V1
rownames(mat) = feature.names$V2

mat.1 <- mat[!duplicated(rownames(mat)), ]
obj <- CreateSeuratObject(counts = mat.1,project = "mouse.brain",assay = "RNA")
metadata <- read.table("metaData_scDevSC.txt",sep = "\t",head = T,quote = "")
metadata <- metadata[-1,]
...
saveRDS(subset_obj,file = "sc.obj.rds")
